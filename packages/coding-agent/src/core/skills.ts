...
function loadSkillFromFile(
	...
	for (const message of routingResult.errors) diagnostics.push({ type: "warning", message, path: filePath });
	if (routingResult.errors.length) return { skill: null, diagnostics };
	return {
		skill: {
			name,
...
export function formatSkillsForPrompt(
	skills: Skill[],
	optionsOrTool: FormatSkillsForPromptOptions | "read" | "bash" = {},
): string {
	const options: FormatSkillsForPromptOptions =
		typeof optionsOrTool === "string" ? { loadToolName: optionsOrTool, includeLocations: true } : optionsOrTool;
	...
	} else if (loadToolName === "bash") {
		lines.push("Use bash to load a skill's file when the task matches its description.");
	} else {
