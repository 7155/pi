...
export class AgentSession {
	...
	private _installAgentForcedPromptProjection(): void {
		...
		this.agent.transformContext = async (messages, signal) => {
			...
			const forced =
				this._runSystemPromptOptions?.forceSystemPrompt ?? this._baseSystemPromptOptions.forceSystemPrompt;
		...
		try {
			...
			if (this._extensionRunner && savedCompactionEntry) {
				...
				if (extensionResult?.systemPrompt !== undefined) {
					this._baseSystemPromptOptions.forceSystemPrompt = extensionResult.systemPrompt;
					if (this._runSystemPromptOptions)
						this._runSystemPromptOptions.forceSystemPrompt = extensionResult.systemPrompt;
				}
			}
