...
- Always read pi .md files completely and follow links to related docs (e.g., tui.md for TUI API details)`;
	...
	const skillFileReadTool = ["skill_load", "read", "bash"].find((tool) => selectedTools.includes(tool));
	if (skillFileReadTool && skills.length > 0) {
		const skillsPrompt = formatSkillsForPrompt(skills, {
			loadToolName: skillFileReadTool,
			searchToolName: selectedTools.includes("skill_search") ? "skill_search" : undefined,
			includeLocations: skillFileReadTool !== "skill_load",
			includeRevision: skillFileReadTool === "skill_load",
		}).trim();
