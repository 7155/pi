...
describe("Room bootstrap tools", () => {
	...
	it("exposes only the five governed native coding tools to Agent Sessions", () => {
		...
		expect([...definitions.keys()].sort()).toEqual(
			[READ_TOOL_NAME, GREP_TOOL_NAME, FIND_TOOL_NAME, LS_TOOL_NAME, BASH_TOOL_NAME].sort(),
		);
		expect(definitions.has(EDIT_TOOL_NAME)).toBe(false);
		expect(definitions.has(WRITE_TOOL_NAME)).toBe(false);
		expect(registry.governedLoadReceipts()).toEqual([]);
		expect(registry.disclosed()).toEqual([]);
	});
});
