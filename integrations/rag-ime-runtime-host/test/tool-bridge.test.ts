...
describe("BackendToolRegistry", () => {
	...
	it("binds each product Tool call to the active factual Pi assistant loop", async () => {
		let sourceLoopId = "pi:message:assistant:101";
		const fetchMock = vi.fn<typeof fetch>().mockImplementation(async () =>
		...
		try {
			...
			await definition.execute(
				"call-first-loop",
				{ op: "post", kind: "progress", content: "第一步" } as never,
				undefined,
				undefined,
				{} as never,
			);
			sourceLoopId = "pi:message:assistant:202";
			await definition.execute(
				"call-follow-up-loop",
				{ op: "post", kind: "result", content: "收尾" } as never,
				undefined,
				undefined,
				{} as never,
			);
			...
			const requests = fetchMock.mock.calls.map(([, init]) => {
				const body = (init as unknown as { body?: unknown } | undefined)?.body;
				return JSON.parse(String(body)) as Record<string, unknown>;
			});
			expect(requests.map((request) => request.sourceLoopId)).toEqual([
...
describe("product gateway request bounds", () => {
	...
	it("keeps Room delegation alive for the Room-turn budget instead of the ordinary 30 second gateway bound", async () => {
		vi.useFakeTimers();
		...
		try {
			const definition = createBackendToolDefinition(
				{
					sessionId: "session-room-delegation",
					registry: new BackendToolRegistry(),
					gatewayUrl: "http://127.0.0.1:8768/api/agent/tool/execute",
				},
				tool({ name: "room_partner" }),
			...
			const pending = definition.execute(
				"call-room-delegation",
				{ op: "delegate_batch", assignments: [] } as never,
				undefined,
				undefined,
				{} as never,
			);
			...
			resolveFetch?.(
				new Response(JSON.stringify({ ok: true, result: { operation: "delegate_batch", results: [] } }), {
					status: 200,
					headers: { "Content-Type": "application/json" },
				}),
			);
