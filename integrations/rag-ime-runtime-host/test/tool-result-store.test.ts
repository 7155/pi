	...
	it("projects snapshot-bound edit and write schemas with truthful mutation receipts", async () => {
		...
		const responses = [
			...
			},
			{
				...
				workDocumentRegistration: {
					document: {
						documentId: "workdoc_123",
						authorityKey: "room_work_item:room-work:1",
						authorityRevision: 2,
						documentRevision: 1,
						path: "docs/agent/work/active/room_work_item/workdoc_123.md",
					},
				},
			},
			{
				...
				error: "WorkDocumentError: authority revision is stale",
			},
		];
		const fetchMock = vi.fn<typeof fetch>().mockImplementation(
			async () =>
		...
		const failedWriteResult = await definitions
			.get("write")
			?.execute(
				"call-write-stale-authority",
				{
					path: "stale.md",
					resourceRevision: "missing",
					content: "stale\n",
		...
		expect(writeResult?.content[0]).toEqual({
			...
			text: [
				...
				"path=/workspace/created.ts",
				`postimageSha256=${"c".repeat(64)}`,
				"workDocument.documentId=workdoc_123",
				"workDocument.authorityKey=room_work_item:room-work:1",
				"workDocument.authorityRevision=2",
				"workDocument.documentRevision=1",
				"workDocument.canonicalPath=docs/agent/work/active/room_work_item/workdoc_123.md",
				"后续对此 WorkDocument 的 read/write 必须使用 workDocument.canonicalPath，不再使用首次请求路径。",
			].join("\n"),
		});
		expect(failedWriteResult?.content[0]).toEqual({
			type: "text",
			text: "操作未执行\nerror=WorkDocumentError: authority revision is stale",
		});
