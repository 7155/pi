...
describe("session context refresh", () => {
	...
	it("rebases every compaction epoch with exactly one bounded Room recovery snapshot", async () => {
		...
		let rebasedPrompt = providerContextJournal.project("基础提示词", {
			sessionContext,
			roomContext: storedRoomRecoveryContext,
			transientContext: "",
		});
		expect(rebasedPrompt).toContain("## 第一次压缩后任务记忆");
		for (const fact of roomRecoveryFacts) {
			expect(rebasedPrompt.split(fact)).toHaveLength(2);
		}
		expect(rebasedPrompt).not.toContain("## Room 当前任务");
		expect(rebasedPrompt).not.toContain("## 子任务记忆");
		expect(rebasedPrompt).not.toContain("过期 Room 恢复包");
		...
		for (const [ordinal, expectedContext] of [
			...
			rebasedPrompt = providerContextJournal.project("基础提示词", {
				sessionContext,
				roomContext: storedRoomRecoveryContext,
				transientContext: "",
			});
	...
	it("treats an empty refreshed Session context as an authoritative clear", async () => {
		...
		let roomRecoveryContext = "stale Room recovery";
		let refreshCount = 0;
		const handlers = new Map<string, (event: any, context?: any) => Promise<unknown>>();
		const fetchMock = vi.fn().mockResolvedValue({
			ok: true,
		...
		const extension = createSessionContextRefreshExtension({
			...
			getRoomRecoveryContext: () => roomRecoveryContext,
			setRoomRecoveryContext: (value) => {
				roomRecoveryContext = value;
			},
			getRecentMessages: () => [],
		...
		expect(compactResult).toBeUndefined();
		const nextPrompt = providerContextJournal.project("base prompt", {
			roomContext: roomRecoveryContext,
			sessionContext,
			transientContext: "",
		});
		expect(nextPrompt).toContain("authoritative Room recovery");
		expect(nextPrompt).not.toContain("stale Session memory");
		expect(nextPrompt).not.toContain('type="session_memory"');
		expect(providerContextJournal.snapshot()).toMatchObject({
			epoch: 2,
			epochReason: "compaction",
			entryCount: 1,
