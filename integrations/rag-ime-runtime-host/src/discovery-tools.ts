...
function routingSearchScore(
	...
	notFor: readonly string[] | undefined,
	...
	const positive = when ?? [];
	let score = searchScore(normalizedQuery, normalizedName, positive.join(" "), supportingText);
	const positiveCoverage = Math.max(0, ...positive.map((item) => fragmentCoverage(normalizedQuery, item)));
		...
		factory(pi) {
			...
			pi.on("before_agent_start", async (event) => {
				const missingBodies = [...loadedSkillBodies]
					.filter(([name]) => !event.systemPrompt.includes(`<loaded_skill name="${escapeXmlAttribute(name)}"`))
					.map(([, body]) => body);
				const toolCatalog = event.systemPrompt.includes(TOOL_CATALOG_MARKER)
					? ""
				...
				if (missingBodies.length === 0 && !toolCatalog) return undefined;
				return {
					systemPrompt: [event.systemPrompt, ...missingBodies, toolCatalog].filter(Boolean).join("\n\n"),
				};
			});
			pi.on("session_compact", async (_event, ctx) => {
				restoreLoadedSkillState(ctx.sessionManager.getBranch());
				// Pi 0.84 owns compaction settlement. Loaded Skill bodies are
				// re-injected by the next before_agent_start hook instead of
				// attempting to override session_compact's (void) result.
			});
			pi.registerTool({
				...
				pi.registerTool({
					...
					execute: async (toolCallId, args, signal) => {
						...
						return {
							...
							details,
							addedToolNames: prepared.map((item) => item.tool.name),
						};
					},
				});
			}
