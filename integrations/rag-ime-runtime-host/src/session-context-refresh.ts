	...
	return (pi) => {
		...
		pi.on("session_compact", async (event, ctx) => {
			try {
				...
				options.providerContextJournal.beginEpoch(
					"compaction",
					ctx.getSystemPrompt(),
					{
				...
				// Pi 0.84 owns the compaction settlement and ignores handler returns.
				// The refreshed context is projected by the normal context extension
				// at the next before_agent_start boundary.
			} finally {
